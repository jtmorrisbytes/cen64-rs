    ...
    const uint16_t *src, unsigned element)
{
  ...
  case 6:
  case 7:
#ifndef _MSC_VER /// does not work on msvc
  ...
  #if !defined(NDEBUG)
    __debugbreak();   // MSVC equivalent of a trap/breakpoint
  #else
    __assume(0);      // MSVC equivalent of unreachable
  #endif
#else
  /* Original GCC/Clang Logic */
  #if defined(NDEBUG)
